...
`go generate` requires a collection of node modules which are installed via npm. If you don't have npm, it comes with node. To get it via homebrew:
`brew install node`
...
- install the new modules locally
